 ...
 *
 * @example
 * <MonsterActions onAction={(action) => console.log(action)} monsterId="123" />
 */
export function MonsterActions ({ onAction, monsterId }: MonsterActionsProps): React.ReactNode {
  ...
  const { trackFeedMonster, trackPlay } = useQuestProgress()
  ...
  const handleAction = (action: MonsterAction): void => {
    ...
    if (action === 'feed') {
      trackFeedMonster()
    } else if (action === 'comfort' || action === 'hug' || action === 'wake') {
      trackPlay()
    }
  }
